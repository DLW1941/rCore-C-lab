...
// The allocator has sz_info for each size k. Each sz_info has a free list,
// an array alloc to keep track which blocks have been allocated,
// and an split array to to keep track which blocks have been split.
// The arrays are of type char (which is 1 byte), but the allocator uses 1 bit per block 
...
// Free memory pointed to by p, which was earlier allocated using bd_malloc.
void
bd_free(void *p) {
  void *q;
  int k;
...
bd_mark(void *start, void *stop)
{
  ...
  if (((usize) start % LEAF_SIZE != 0) || ((usize) stop % LEAF_SIZE != 0))
    panic("bd_mark");
...
// Initialize the free lists for each size k.  For each size k, 
// there are only two pairs that may have a buddy that should be on free list:
...
bd_init(void *base, void *end) {
  char *p = (char *) ROUNDUP((usize)base, LEAF_SIZE);
  int sz;
  ...
  // since we will not split blocks of size k = 0, the smallest size.
  ...
  p = (char *) ROUNDUP((usize) p, LEAF_SIZE);

  // done allocating; mark the memory range [base, p) as allocated, 
